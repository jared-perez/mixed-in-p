...
- **`python scripts/visual_pass.py` before a release**: a translated label that overflows its widget still passes every assertion about its text, so this renders all twelve languages offscreen and diffs required-vs-allocated width per widget against English. Two things it taught. **Persist the language to config, don't just install the translator** — the sidebar shrinks one Russian label based on `load_config().language`, so a harness that skips that step invents six sidebar defects that do not exist. And **report as a diff against English, not absolutely**: English alone measures ~24 findings that are metric jitter, which would bury the real ones. Ground-truth anything it flags against a screenshot before acting — its padding estimates produce occasional false positives (`プレイリスト` was reported 22px over and renders fine).
