...
- **`font-size` in QSS must go on the widget selector, not a sub-control, if anything measures `widget.font()`.** `QHeaderView::section { font-size: … }` is honoured when the section is painted — the header visibly grows — but never reaches `header.font()`, and `SeparatorHeaderView._title_font()` derives from exactly that, which is what the playlist's word-fit column widths are measured with. The result was a larger header word in an unchanged, too-narrow column. Use `QHeaderView { font-size: … }`. Related: **row height does not follow the font either** — rows sit at the vertical header's `defaultSectionSize` and nothing re-measures them, so scale that explicitly. `resizeRowsToContents()` is the tempting fix and the wrong one: it sizes to the delegate hint *plus* the QSS `::item` padding, which took the playlist from 30px rows to 51px **at the size everyone already uses**, i.e. it redesigns the app for users who never touched the setting.
