...
- **Waveform colour is analysis → palette → canvas, and the analysis is normalisation-free** (band *shares*, centroid in Hz; `spectral_columns` in `waveform_worker.py`), so the mode and Fixed/This-track settings re-map in `waveform_palette.py` without re-analysing, and only Frequency bands/Tone ever run the FFT. Mix to mono with `mono_mix`, never `pcm.mean(axis=1)` — numpy reduces a length-2 fast axis 18× slower.
...
- **`QAbstractButton.click()` is not the user's click**: a real press/release runs `QAbstractButtonPrivate::click()`, which raises `blockRefresh` so `setChecked` skips `checkStateSet()`, and `QCheckBox` then calls its own non-virtually — past any Python override. Hang a button's own housekeeping off **both** `checkStateSet()` (for state loaded inside `blockSignals`) and `toggled` (for the user), and drive such a test with `qtbot.mouseClick`, since `.click()` passes either way (`ToggleSwitch`).
