...
- **Conversion quality only ever goes down** (`src/conversion/result.py`): into another format only `raises_quality` is refused (an `error`); into the same format only a strict `is_quality_downgrade` runs, anything else is `skipped`; MP3 is exempt from both. Three call sites must agree or a row reads "Ready" and Convert does nothing: `convert_file`, the CLI dry run, and `ConversionPanel._verdict`. A `None` rate/depth means "Keep source" and must survive every hop (a Signal `object`, not `int`; `_optional_int` in config) — an `int` path silently turns it into 0.
